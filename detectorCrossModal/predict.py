...
def extract_text_inputs(text, tokenizer, max_len=128):
...
def predict(video_path, model, tokenizer, device, frame_interval=30, max_frames=10, max_len=128):
    ...
    with open(text_path, 'r') as text_file:
        text = text_file.read()
    input_ids, attention_mask = extract_text_inputs(text, tokenizer, max_len)
    ...
    aux_features = torch.tensor(aux_features, dtype=torch.float).unsqueeze(0).to(device)
