...
class MultiModalModel(nn.Module):
    def __init__(self, text_model, num_labels, class_weights, aux_feature_dim=4):
        ...
        self.classifier = nn.Linear(768 * 3 + aux_feature_dim, num_labels)
        self.loss_fn = nn.CrossEntropyLoss(weight=class_weights)

        self.dropout = nn.Dropout(p=0.3)  # Adding dropout for regularization

    def forward(self, input_ids, attention_mask, audio=None, video=None, aux_features=None, labels=None):
        with autocast():
            ...
            if aux_features is None:
                aux_features = torch.zeros(
                    text_features_embedding.size(0),
                    self.aux_feature_dim,
                    device=text_features_embedding.device,
                )
            ...
            combined_features = torch.cat(
                (text_features_embedding[:, 0, :], audio_attn[:, 0, :], video_attn[:, 0, :], aux_features),
                dim=1,
            )
